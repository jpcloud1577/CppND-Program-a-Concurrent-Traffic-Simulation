...
{
   //TL2.3: Get the entries from the front of _promises and _vehicles. 
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    //TL2.2 - add the new vehicle to the waiting line by creating a promise, 
    //a corresponding future and then adding both to _waitingVehicles. 
    //Then wait until the vehicle has been granted entry.

    std::promise<void> prmsNewvehicle;
    std::future<void> futureNewvehicle = prmsNewvehicle.get_future();

    _waitingVehicles.pushBack(vehicle, std::move(prmsNewvehicle));

    futureNewvehicle.wait();
